...
SELECT EMPLOYEE_ID
     , FIRST_NAME
     , LAST_NAME
     , E.DEPARTMENT_ID      -- E/D 아무거나 적어도 차이 없음!! 
                            -- 2개의 테이블에 모두 있는 칼럼(이름이 같은 칼럼)은 반드시 테이블(오너)을 명시해야 한다.
     , DEPARTMENT_NAME
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E  -- *1:M 1을 앞에, M를 뒤에 쓰기!!!/ 답은 똑같음 성능 차이// 양이적은 것이 앞, 양이 큰게 뒤
...
SELECT EMPLOYEE_ID
     ...
     , D.DEPARTMENT_ID
     , DEPARTMENT_NAME
  FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E -- 왼쪽 테이블 (EMPLOYEES)의 모든 데이터를 조회하시오.(부서번호가 없는 사원도 조회하시오.)
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
