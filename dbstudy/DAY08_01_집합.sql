/*
    ...
    2. 모든 SELECT 절의 칼럼 순서와 타입이 일치해야한다.
    3. 집합 연산자
...
-- 사원테이블과 부서 테이블에 존재하는 모든 부서 번호를 조회하기
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
 UNION
SELECT DEPARTMENT_ID  -- 여기서 NULL값 들어감
  FROM EMPLOYEES;
  
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS  --27번 270까지 
 UNION ALL
SELECT DEPARTMENT_ID  -- 나머지 밑에
  FROM EMPLOYEES;
  
  
-- 교집합, 사원테이블과 부서테이블에 모두 존재하는 부서번호 조회하기
--(사원들이 근무 중인 부서번호만 조회하기)  -- 조인으로 풀면 내부 조인으로 가능
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
INTERSECT
SELECT DEPARTMENT_ID
...
-- 부서테이블에 존재하지만 사원테이블에 존재하지 않는 부서 번호 조회하기
-- 유령부서 조회하기)
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
MINUS
...
WITH
    MY_SUBQUERY(N, TOTAL) AS (         -- N, TOTAL은 MY_SUBQUERY의 칼럼을 의미한다.
        SELECT 1, 1                    -- N=1, TOTAL=1을 의미하는 초기화 서브쿼리/시작은 1, 1이다.
          FROM DUAL
         UNION ALL
...
-- MANAGER가 몇 명인가에 따른 LEV 표시하기
-- MANAGER가 0명이다 : LEV = 1
-- MANAVER가 1명이다 : LEV = 2

WITH
    MY_SUBQUERY(LEV, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID) AS (
    -- 초기값(LEV=1)을 지정하는 서브쿼리
    SELECT 1, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, MANAGER_ID
      FROM EMPLOYEES
     WHERE MANAGER_ID IS NULL
     -- 반복해서 호출되는 서브쿼리
     UNION ALL -- 그냥 , ALL 상관 없음
     SELECT M.LEV + 1 AS LEV, E.EMPLOYEE_ID, E.FIRST_NAME, E.LAST_NAME, E.MANAGER_ID
      FROM EMPLOYEES E INNER JOIN MY_SUBQUERY M  --EMPLOYEES 테이블에 LEVEL 없음 
