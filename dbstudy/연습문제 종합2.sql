...
SELECT O.ORDER_ID AS 주문번호, B.BOOK_ID AS 책번호, BOOK_NAME AS 책이름
  FROM BOOK_T B INNER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID
 WHERE O.ORDER_DT BETWEEN TO_DATE('20/07/05', 'YY/MM/DD') AND TO_DATE('20/07/09', 'YY/MM/DD')
 ORDER BY ORDER_DT;
...
SELECT O.ORDER_ID AS 구매번호, C.CUST_NAME AS 구매자, B.BOOK_NAME AS 책이름, B.PRICE * O.AMOUNT AS 총구매액, O.ORDER_DT AS 주문일자
 FROM BOOK_T B INNER JOIN ORDER_T O
   ...
   ON C.CUST_ID = O.CUST_ID
WHERE O.ORDER_DT NOT BETWEEN TO_DATE('20/07/04', 'YY/MM/DD') AND TO_DATE('20/07/07', 'YY/MM/DD');
   
/*
SELECT ORDER_DT
 FROM ORDER_T
WHERE ORDER_DT BETWEEN TO_DATE('20/07/04', 'YY/MM/DD') AND TO_DATE('20/07/07', 'YY/MM/DD');
*/
...
SELECT CUST_NAME AS 고객명, BOOK_NAME AS 책이름, ORDER_DT AS 주문일자
 FROM BOOK_T B INNER JOIN ORDER_T O
   ON B.BOOK_ID = O.BOOK_ID INNER JOIN CUSTOMER_T C
...
---------------
SELECT NULL AS 주문번호, BOOK_ID AS 책번호, BOOK_NAME AS 책이름
  FROM BOOK_T 
...
SELECT CUST_NAME AS 고객이름, BOOK_NAME AS 책이름, PRICE AS 가격
  FROM BOOK_T B LEFT OUTER JOIN ORDER_T O
...
SELECT C.CUST_NAME AS 고객이름, B.BOOK_NAME AS 책이름, B.PRICE AS 가격
  FROM CUSTOMER_T C LEFT OUTER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID RIGHT OUTER JOIN BOOK_T B
    ON B.BOOK_ID = O.BOOK_ID
 WHERE B.PRICE = (SELECT MAX(PRICE) FROM BOOK_T);
...
SELECT C.CUST_NAME AS 고객명, COUNT(O.ORDER_ID) AS 구매도서수
 ...
 GROUP BY C.CUST_ID, C.CUST_NAME ;  -- 고객이름의 경우 종복이 많기 때문에 CUST_ID도 같이 묶어야 오류 없음
 
-- '김연아'가 구매한 책이 몇 종인가? (피겨의 정석을 2번 주문했다면 주매한 책은 1종)
 SELECT C.CUST_NAME AS 고객명, COUNT(DISTINCT O.BOOK_ID) AS 구매도서수
 FROM CUSTOMER_T C INNER JOIN ORDER_T O
   ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
...
SELECT CUST_NAME AS 고객명, COUNT(PUBLISHER) AS 출판사수
  FROM CUSTOMER_T C INNER JOIN ORDER_T O 
...
SELECT CUST_NAME AS 고객명, COUNT(DISTINCT B.PUBLISHER) AS 출판사수
 FROM CUSTOMER_T C INNER JOIN ORDER_T O 
    ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
    ON B.BOOK_ID = O.BOOK_ID
...
SELECT CUST_NAME AS 고객명, SUM(PRICE * AMOUNT) AS 총구매액
  FROM CUSTOMER_T C INNER JOIN ORDER_T O 
    ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
    ON B.BOOK_ID = O.BOOK_ID
 GROUP BY C.CUST_ID, C.CUST_NAME;
...
SELECT CUST_NAME AS 고객명, NVL(SUM(PRICE * AMOUNT),0) AS 총구매액, COUNT(ORDER_ID) AS 구매횟수
  FROM BOOK_T B LEFT OUTER JOIN ORDER_T O
...
SELECT A.CUST_NAME AS 고객명
  ...
  FROM (SELECT RANK() OVER(ORDER BY SUM(B.PRICE *O.AMOUNT) DESC) AS RK
          ...
          FROM CUSTOMER_T C INNER JOIN ORDER_T O
            ON C.CUST_ID = O.CUST_ID INNER JOIN BOOK_T B
            ON B.BOOK_ID = O.BOOK_ID
        GROUP BY C.CUST_ID, CUST_NAME) A
