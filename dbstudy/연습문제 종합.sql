...
SELECT USER_NO, USER_ID, USER_MOBILE1, USER_MOBILE2
  FROM USER_T
 WHERE USER_MOBILE2 = (SELECT USER_MOBILE2
                         FROM USER_T
                        WHERE USER_MOBILE2 LIKE '5%');
...
SELECT USER_NO, USER_ID, USER_REGDATE
  FROM USER_T
 WHERE USER_REGDATE IN  (SELECT  USER_REGDATE
                                         FROM USER_T
...
--그룹핑 이전 : WHERE
--그루핑 이후 : HAVING - 지역별 사용자수가 2명 이상인 경우
...
SELECT USER_ID AS 아이디, COUNT(B.BUY_NO) AS 구매횟수
...
-- 모든 사용자 조회가 목적!
SELECT USER_ID AS 아이디, U.USER_NAME AS 고객명, COUNT(B.USER_NO) AS 구매횟수
...
-- 코드와 이름이 모두 동일하면 하나의 데이터로 봄, 안전장치/ GROUP BY에 명시한 것만 SELECT으로 보내면 됨
-- 벨트 외부조인

-- 12. 카테고리가 '전자'인 제품을 구매한 고객의 구매내역(하나당 가격 * 개수)을 조회하시오.
-- 고객명  제품명  구매액
-- 강호동  노트북  1000
-- 김용만  모니터  200
-- 박수홍  모니터  1000
...
SELECT U.USER_NAME AS 고객명, P.PROD_NAME AS 제품명, PROD_PRICE*BUY_AMOUNT AS 구매액
  FROM PRODUCT_T P LEFT OUTER JOIN BUY_T B
    ON P.PROD_CODE = B.PROD_CODE  LEFT OUTER JOIN USER_T U
...
SELECT  U.USER_NAME  AS 고객명, P.PROD_NAME AS 제품명, P.PROD_PRICE * B.BUY_AMOUNT AS 구매액
  FROM USER_T U INNER JOIN BUY_T B
    ON B.USER_NO = U.USER_NO INNER JOIN PRODUCT_T P
    ON P.PROD_CODE = B.PROD_CODE
 WHERE PROD_CATEGORY = '전자';

-- 13. $ 제품을 구매한 이력이 있는(INNER JOIN) 고객의 아이디, 고객명, 구매횟수, 총구매액을 조회하시오.
-- 아이디  고객명  구매횟수  총구매액
-- PSH     박수홍  3         1860
-- KYM     김용만  1         200
-- KJD     김제동  1         75
...
SELECT U.USER_ID AS 아이디
 ...
 FROM USER_T U INNER JOIN BUY_T B
   ...
   ON P.PROD_CODE = B.PROD_CODE
GROUP BY  U.USER_ID, U.USER_NAME; -- ID와 NAME이 같으면 하나의 데이터로 인식
...
-- 18. 구매번호가 가장 큰 구매내역을 삭제하시오.  -INSERT, DELETE 도 SUBQUERY 이용 가능!!
--구매번호는 시퀀스가 만들고 있음.. 
DELETE 
  FROM BUY_T
 ...
 -- 시퀀스 번호 어디까지 생성되는지 알 수 있는 방법 CURRVAL 사용하려면 NEXTVAL 사용해야지만 쓸 수 있음!
SELECT BUY_SEQ.CURRVAL
  FROM DUAL;

-- 아래 쿼리는 비추천.
-- 시퀀스를 사용하였으나, INSERT 자체가 실패한 경우 가장 큰 구매 번호와 CURRVAL값은 다르다.
-- 언제나 일치하지 않아서 권장하지 않음
DELETE 
  FROM BUY_T
...
DELETE
 FROM BUY_T
WHERE USER_NO = 5;  -- 성공
--COMMIT; 여기 있으면 안됨!!!  - 작업이 하나면 커밋도 하나! , 두개면 안됨!

DELETE  
  FROM USER_T
 WHERE USER_NO = 5;  -- 실패 -> 롤백으로 돌아 갈 수 있음 그러면 위 성공이 취소 될 수 있음... 둘 다 성공해야 하므로 뒤에꺼에만 COMMIT;
 COMMIT;
