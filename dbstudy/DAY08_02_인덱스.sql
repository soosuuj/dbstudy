...
SELECT *
  FROM EMPLOYEES
 WHERE ROWID = 'AAAR+MAAHAAAADOAAE';
...
/*
    ...
    3. 계정 유형별로 관리하다.
        1) DBA_  - 가장 많은 정보
        2) USER_  -USER가 가지고 있는 정보
        3) ALL_  - USER가 접근 
*/

-- 인덱스 정보가 저장되어 있는 데이터 사전(이미 만들어진 테이블)-메타 데이터라고도 함..
--DBA_INDEXES, USER_INDEXES , ALL_INDEXES
SELECT * FROM ALL_INDEXES;
SELECT * FROM USER_INDEXES;
SELECT * FROM DBA_INDEXES; -- HR계정이 접근할 수 없어 볼 수 없음.

-- 인덱스가 설정된 칼럼 정보가 저장되어 있는 데이터 사전
-- DBA_IND_COLUMNS, USER_IND_COLUMNS, IND_COLUMNS
SELECT *FROM USER_IND_COLUMNS;
SELECT *FROM USER_IND_COLUMNS WHERE TABLE_NAME = 'EMPLOYEES'; -- 사원 테이블에 있는 정보만 조회
